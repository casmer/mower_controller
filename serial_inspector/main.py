...
class GUI:
    ...
    def __init__(self, title):
        ...
        self.scanButton.grid(column=1, row=1, sticky=(W, N, E, S))

        self.portsOptionMenu.configure( padx=padding, pady=padding)
        self.portsOptionMenu.grid(column=2, row=1, sticky=(N, W, E, S))

        self.baudRatesOptionMenu.configure( padx=padding, pady=padding)
        self.baudRatesOptionMenu.grid(column=3, row=1, sticky=(N, E))

        self.connectButton.configure( padx=padding, pady=padding)
        self.connectButton.grid(column=4, row=1, sticky=(N, E))
        ...
        for channel in range(0,16):
            ...
            self.radio_channels.append(tk.Label(self.topFrame, textvariable=self.radio_channel_values[channel]))
    
            self.radio_channels[-1].grid(column=channel%4 + 1, row=2 + channel//4, sticky=(N, E,W,S))
        
        self.textBox.configure( padx=padding, pady=padding)
        self.textBox.grid(column=1, row=6, columnspan=4, sticky=(N, E,W,S))
        ...
        for col_num in range(1, 5):
            self.topFrame.columnconfigure(col_num, weight=1)
